...
> [!IMPORTANT]
> Only one framework skill should be active per project. Check `talos-control/state.yaml`
...
1. Read `talos-control/state.yaml` → `stack.backend` to select the correct framework skill.
...
6. Update `talos-control/implementation_steps.md` as steps complete.

## Constraints

- Must follow contracts defined in `backend_contract.md` output.
